...
def animal_name(address)
	...
	pairs = digest.scan(/.{1,2}/)

	bytes = pairs.map{ |p| p.to_i(16) }
	...
	while(i < segSize * 3) do
		segments.append(bytes.slice(i, segSize))
		i += segSize
	end
	...
	segments[segments.length-1] = lastSeg.append(bytes.slice(3 * segSize))

	compressed = segments.map{|x| x.reduce{|acc, curr| acc ^ curr }}

	adj = adjs[compressed[0]]
